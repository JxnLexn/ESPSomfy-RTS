...
void JsonFormatter::addElem(uint64_t nval) { sprintf(this->_numbuff, "%llu", (unsigned long long)nval); this->_appendNumber(nullptr); }
...
void JsonFormatter::addElem(const char *name, uint64_t nval) { sprintf(this->_numbuff, "%llu", (unsigned long long)nval); this->_appendNumber(name); }
